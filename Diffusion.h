...
namespace NS
{
	...
	struct Diffusion_U
	{
		...
		const unsigned int Nx, Ny, margin;
		template<class BF>
		Diffusion_U(double Dx, double Dy, double dt, unsigned int Nx, unsigned int Ny, unsigned int margin, const BF &top)
			:Dx(Dx), Dy(Dy), dt(dt), Nx(Nx), Ny(Ny), margin(margin), top(new BF(top)){}
		~Diffusion_U(){delete top;}
		...
		*/
			for(auto &U : Domain(Nx, Ny, margin, 0, Nx, 0, 1))
				cod[U] = dom[U] - dt*(Dx*(dom[U(1, 0)] + dom[U(-1, 0)] - 2*dom[U]) + Dy*(dom[U(0, 1)] - 3*dom[U]));
